...
async def send_log(bot, log_group, peer, ok, error=None):
    """Send clickable log to log group"""
    if not log_group:
        return
    try:
        peer_clean = peer.lstrip("@")
        if ok:
            # Clickable link using HTML
            text = f'✅ Successfully forwarded to: <a href="https://t.me/{peer_clean}">{peer_clean}</a>'
        else:
...
async def do_forward(bot, customer, state):
    ...
    for i, peer in enumerate(groups):
        ...
        try:
            ...
            sent += 1
            await send_log(bot, log_group, peer, True)
        except Exception as e:
            failed += 1
            logging.warning(f"Forward failed {peer}: {e}")
            await send_log(bot, log_group, peer, False, e)

        # Wait 1 minute before next group (except last one)
        if i < len(groups) - 1 and state.get("active"):
            await asyncio.sleep(GROUP_DELAY)

    # Campaign complete — set last_sent to start 30 min countdown
...
async def run_customer_bot(customer):
    ...
    # Schedule loop — checks every minute, triggers campaign every 30 min
