...
def load_session(account_id):
    # Try env var first, then Secret File as fallback
    session_str = os.getenv(f"TG_SESSION_{account_id}")
    if session_str:
        return session_str.strip()
    # Try Render Secret File path
    file_path = f"/etc/secrets/session_{account_id}.txt"
    if os.path.exists(file_path):
        with open(file_path) as f:
            return f.read().strip()
    # Also try generic session.txt for account 1
    if account_id == 1:
        for path in ["/etc/secrets/session.txt", "/sdcard/session_new.txt"]:
