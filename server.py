...
clients, locks = {}, {}
stop_flags = {}  # stop flag per account

def load_session(account_id):
    # 1. Try session file in repo root
    ...
    if len(s) >= 350:
        ...
        print(f"⚠️ Account {account_id}: env var too short ({len(s)} chars) — truncated")
...
async def stop(req: StopRequest, authorization: str | None = Header(default=None)):
    ...
    print(f"🛑 Stop flag set for account {req.accountId}")
    return {"ok": True}
...
async def send(req: SendRequest, authorization: str | None = Header(default=None)):
    ...
    stop_flags[req.accountId] = False  # clear stop flag at start
    results = []

    async with locks[req.accountId]:
        for raw in req.peers:
            # Check stop flag before each group
            if stop_flags.get(req.accountId, False):
                print(f"🛑 Send aborted for account {req.accountId}")
                break
